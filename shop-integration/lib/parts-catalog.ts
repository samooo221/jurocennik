// Pure catalog search + grounding — framework-agnostic, used by both the React
// component (client) and the API route (server). Ported from the standalone
// app's catalog-search.js. It can only ever return parts that exist in the
// catalog, so it cannot invent a part or a price.
...
export const SYNONYMS: Record<string, string[]> = {
  ...
  spoiler: ["spoiler"], bodywork: ["bodywork"], kapotaz: ["bodywork"], karoseria: ["bodywork"],
...
// Normalized search haystack per part, cached without mutating the part.
const haystackCache = new WeakMap<Part, string>();
function haystack(p: Part): string {
  let h = haystackCache.get(p);
  if (h === undefined) {
    h = normalize(
      [p.pn, p.desc, p.groups.join(" "), p.models.join(" "), p.subsystems.join(" ")].join(" ")
    );
...
function facetMatch(p: Part, { model, subsystem, group }: Facets): boolean {
...
// Ranked parts. Empty query + no facets -> [] (UI shows a hint, not all parts).
export function search(parts: Part[], query: string, facets: Facets = {}, limit = 200): Part[] {
  const hasFacets = Boolean(facets.model || facets.subsystem || facets.group);
...
// parts, dropping any that don't exist. byPn is { pn: part }.
export function groundPns(pns: unknown, byPn: Record<string, Part>): Part[] {
  const seen = new Set<string>();
  return (Array.isArray(pns) ? pns : [])
    .map((pn: unknown) => byPn[String(pn)])
    .filter((p): p is Part => Boolean(p) && !seen.has(p.pn) && Boolean(seen.add(p.pn)));
}

export const VAT = 1.23; // Slovak DPH 23% — headline price = net × VAT; net shown beneath
